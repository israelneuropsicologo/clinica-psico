...
## Fase 29: Correções de Transcrição e Menu de Ações (Maio 2026)
- [x] Corrigir refetch automático após transcrição
  - **Problema**: Transcrição era concluída mas não aparecia na interface
  - **Solução**: Adicionar `onSuccess: () => refetchRecordings()` na mutation
  - **Implementação**: client/src/pages/PatientDetail.tsx (linha 128)
  - **Resultado**: Lista de gravações atualiza automaticamente após transcrição

- [x] Adicionar menu de ações (3 pontinhos) para cada gravação
  - **Componente**: DropdownMenu com MoreVertical icon (⋮)
  - **Opções do menu**:
    * Copiar Transcrição (quando disponível)
    * Baixar Áudio
    * Excluir Gravação
  - **Implementação**: client/src/pages/PatientDetail.tsx (linhas 475-510)
  - **Imports adicionados**: MoreVertical, Copy, Download icons + DropdownMenu components
  - **Teste**: Menu visível e funcional na interface

- [x] Toast de sucesso ao concluir transcrição
  - Mensagem: "Transcrição concluída!"
  - Exibido automaticamente após refetch

- [x] Teste: Validar transcrição e menu de ações
  - Gravações com status "Transcrito" exibem corretamente
  - Menu de ações acessível para cada gravação
  - Refetch automático funcionando
