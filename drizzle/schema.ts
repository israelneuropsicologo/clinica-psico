...
// ─── Medical Records Timeline (Prontuário Dinâmico - Linha do Tempo) ─────────
export const medicalRecordsTimeline = mysqlTable("medical_records_timeline", {
  id: int("id").autoincrement().primaryKey(),
  patientId: int("patientId").notNull(), // FK → patients.id
  sessionId: int("sessionId"), // FK → sessions.id (opcional)
  userId: int("userId").notNull(), // FK → users.id (psicólogo)
  entryType: mysqlEnum("entryType", ["session_note", "clinical_evolution", "intervention", "assessment", "treatment_plan", "discharge_summary"]).notNull(),
  title: varchar("title", { length: 255 }).notNull(),
  content: text("content").notNull(), // Rich text HTML
  clinicalEvolution: text("clinicalEvolution"), // Evolução clínica automática
  interventionSummary: text("interventionSummary"), // Resumo de intervenção
  attachments: text("attachments"), // JSON array de documentos anexados
  tags: text("tags"), // JSON array de tags/categorias
  isPrivate: boolean("isPrivate").default(false).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type MedicalRecordsTimeline = typeof medicalRecordsTimeline.$inferSelect;
export type InsertMedicalRecordsTimeline = typeof medicalRecordsTimeline.$inferInsert;

// ─── Technical Documents (Documentação Técnica - Atestados, Laudos, Pareceres) ─
export const technicalDocuments = mysqlTable("technical_documents", {
  id: int("id").autoincrement().primaryKey(),
  patientId: int("patientId").notNull(), // FK → patients.id
  userId: int("userId").notNull(), // FK → users.id (psicólogo)
  documentType: mysqlEnum("documentType", ["certificate", "report", "opinion", "referral", "prescription", "evaluation"]).notNull(),
  title: varchar("title", { length: 255 }).notNull(),
  templateId: varchar("templateId", { length: 64 }), // ID do template usado
  content: text("content").notNull(), // HTML renderizado
  pdfUrl: varchar("pdfUrl", { length: 1024 }), // URL do PDF gerado
  pdfKey: varchar("pdfKey", { length: 512 }), // S3 key do PDF
  status: mysqlEnum("status", ["draft", "pending_review", "approved", "signed", "archived"]).default("draft").notNull(),
  signatureDate: bigint("signatureDate", { mode: "number" }), // UTC ms
  issuedDate: bigint("issuedDate", { mode: "number" }).notNull(), // UTC ms
  expiryDate: bigint("expiryDate", { mode: "number" }), // UTC ms (opcional)
  metadata: text("metadata"), // JSON com dados adicionais (profissional, CRP, etc)
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
...
// ─── Anamnesis & Intake (Anamnese e Triagem) ──────────────────────────────────
export const anamnesisIntake = mysqlTable("anamnesis_intake", {
  id: int("id").autoincrement().primaryKey(),
  patientId: int("patientId").notNull(), // FK → patients.id
  userId: int("userId").notNull(), // FK → users.id (psicólogo)
  intakeType: mysqlEnum("intakeType", ["initial_assessment", "follow_up", "reassessment"]).default("initial_assessment").notNull(),
  
  // Dados demográficos
  maritalStatus: varchar("maritalStatus", { length: 64 }),
  numberOfChildren: int("numberOfChildren"),
  livingArrangement: varchar("livingArrangement", { length: 255 }),
  ...
  pastPsychiatricHistory: text("pastPsychiatricHistory"),
  familyHistory: text("familyHistory"),
  
  // Hábitos e estilo de vida
  substanceUse: text("substanceUse"), // Álcool, drogas, tabaco
  sleepPatterns: text("sleepPatterns"),
  dietAndExercise: text("dietAndExercise"),
  stressFactors: text("stressFactors"),
  
  // Avaliação mental
  mentalStatusExam: text("mentalStatusExam"),
  moodAndAffect: varchar("moodAndAffect", { length: 255 }),
  cognitiveFunction: text("cognitiveFunction"),
  suicidalityAssessment: text("suicidalityAssessment"),
  
  // Diagnóstico e plano
  preliminaryDiagnosis: text("preliminaryDiagnosis"),
  treatmentPlan: text("treatmentPlan"),
  recommendedTherapy: varchar("recommendedTherapy", { length: 255 }),
  
  // Respostas do formulário inteligente (JSON)
  questionnaireResponses: text("questionnaireResponses"), // JSON com respostas
  
  status: mysqlEnum("status", ["in_progress", "completed", "archived"]).default("in_progress").notNull(),
  completedAt: bigint("completedAt", { mode: "number" }), // UTC ms
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type AnamnesisIntake = typeof anamnesisIntake.$inferSelect;
export type InsertAnamnesisIntake = typeof anamnesisIntake.$inferInsert;
...
export const treatmentPlans = mysqlTable("treatment_plans", {
  ...
  patientId: int("patientId").notNull(), // FK → patients.id
  userId: int("userId").notNull(), // FK → users.id (psicólogo)
  anamnesisId: int("anamnesisId"), // FK → anamnesis_intake.id
  
  title: varchar("title", { length: 255 }).notNull(),
  goals: text("goals"), // Objetivos do tratamento (JSON array)
  interventions: text("interventions"), // Intervenções planejadas (JSON array)
  expectedDuration: int("expectedDuration"), // Duração esperada em sessões
  frequency: varchar("frequency", { length: 64 }), // Frequência (ex: semanal)
  
  startDate: bigint("startDate", { mode: "number" }).notNull(), // UTC ms
  expectedEndDate: bigint("expectedEndDate", { mode: "number" }), // UTC ms
  actualEndDate: bigint("actualEndDate", { mode: "number" }), // UTC ms (quando finalizado)
  
  status: mysqlEnum("status", ["active", "paused", "completed", "discontinued"]).default("active").notNull(),
  notes: text("notes"),
